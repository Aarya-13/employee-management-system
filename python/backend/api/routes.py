...
async def create_employee(employee: EmployeeCreate):
    ...
    """
    try:
        print(f"DEBUG: create_employee called with: {employee}")
        employee_dict = employee.model_dump()
        print(f"DEBUG: Employee dict: {employee_dict}")
        created_employee = operations.create_employee(employee_dict)
        print(f"DEBUG: Created employee: {created_employee}")
        
        if created_employee:
            response = EmployeeResponse(**created_employee.to_dict())
...
async def get_all_employees():
    ...
    try:
        ...
        result = [EmployeeResponse(**emp.to_dict()) for emp in employees]
        print(f"DEBUG: Converted to response models: {result}")
        return result
    except Exception as e:
        print(f"DEBUG: Error in get_all_employees: {str(e)}")
        import traceback
        traceback.print_exc()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
